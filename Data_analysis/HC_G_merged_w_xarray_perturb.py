    ...
    for i in range((end_year-year+1)):
        ...
        for j in range(12):
            ...
            if month < 10: #months 1-9 are specified as "01"-"09" in the file names
                ...
                temps = temps +  np.random.normal(0, temps_u.values)
...
def run_global_n(start_year, end_year, depth_from, depth_to, perturbations):
    """Basically a main; runs all functions defined above."""
    perturb = np.zeros([perturbations, (end_year-start_year+1)*12])
    for i in range(perturbations):
        times, total_HC = calculate_HC_global(start_year, end_year, depth_from, depth_to)
        perturb[i,:] = total_HC
    for i in range(perturbations):
        plt.plot(times, perturb[i, :])
...
#HC = run_global_n(1950, 1953, 0, 500, 5)
