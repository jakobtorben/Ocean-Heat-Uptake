...
def calculate_HC_global(start_year, end_year, depth_from, depth_to):
    ...
    total_HC_u = np.zeros(((end_year-start_year+1)*12))
    ...
    for i in range((end_year-year+1)):
        ...
        for j in range(12):
            ...
            if month < 10: #months 1-9 are specified as "01"-"09" in the file names
                ...
                temps_u = DS.temperature_uncertainty.fillna(0)
                temps_u_interp = temps_u.sel(depth=slice(depth_from,depth_to)).interp(depth = np.arange(depths[0],depths[-1],1), method = 'cubic')
                HC_grid_i_u = temps_u_interp.sel(depth=slice(depth_from,depth_to)).sum("depth")*Cp*rho
                temps_u = temps_u.assign_coords(HC_grid_i_u = HC_grid_i_u)
                temps_u = temps_u.assign_coords(area = area)
                HC_grid_weighted_u = (temps_u.HC_grid_i_u * temps_u.area)
                total_HC_u[count] = HC_grid_weighted_u.isel(time=0).values.sum()
            else:
    ...
    return times, total_HC, total_HC_u
...
def run_global(start_year, end_year, depth_from, depth_to):
    ...
    times, total_HC, total_HC_u = calculate_HC_global(start_year, end_year, depth_from, depth_to)
    ...
    #Uncertainty plus
    np.savetxt("HC_G_u_plus_"+str(start_year)+"_"+str(end_year)+"_"+str(depth_from)+"m_"+str(depth_to)+"m.txt",(times,total_HC + total_HC_u),delimiter=", ")
    np.savetxt("HC_G_anom_u_plus_"+str(start_year)+"_"+str(end_year)+"_"+str(depth_from)+"m_"+str(depth_to)+"m.txt",(times,std_baseline(total_HC + total_HC_u)),delimiter=", ")
    
     #Uncertainty minus
    np.savetxt("HC_G_u_minus_"+str(start_year)+"_"+str(end_year)+"_"+str(depth_from)+"m_"+str(depth_to)+"m.txt",(times,total_HC - total_HC_u),delimiter=", ")
    np.savetxt("HC_G_anom_u_minus_"+str(start_year)+"_"+str(end_year)+"_"+str(depth_from)+"m_"+str(depth_to)+"m.txt",(times,std_baseline(total_HC - total_HC_u)),delimiter=", ")
    
    times2, total_HC_u_plus = moving_avg(times, total_HC + total_HC_u, 12)
    times2, total_HC_u_minus = moving_avg(times, total_HC - total_HC_u, 12)
    ...
    np.savetxt("HC_G_MA_u_plus_"+str(start_year)+"_"+str(end_year)+"_"+str(depth_from)+"m_"+str(depth_to)+"m.txt",(times2, total_HC_u_plus),delimiter=", ")
    total_HC_u_plus = std_baseline(total_HC_u_plus)
    np.savetxt("HC_G_MA_anom_u_plus_"+str(start_year)+"_"+str(end_year)+"_"+str(depth_from)+"m_"+str(depth_to)+"m.txt",(times2,total_HC_u_plus),delimiter=", ")

    
    np.savetxt("HC_G_MA_u_minus_"+str(start_year)+"_"+str(end_year)+"_"+str(depth_from)+"m_"+str(depth_to)+"m.txt",(times2, total_HC_u_minus),delimiter=", ")
    total_HC_u_minus = std_baseline(total_HC_u_minus)
    np.savetxt("HC_G_MA_anom_u_minus_"+str(start_year)+"_"+str(end_year)+"_"+str(depth_from)+"m_"+str(depth_to)+"m.txt",(times2,total_HC_u_minus),delimiter=", ")

    plt.plot(times2, total_HC, color='red')
    plt.plot(times2, total_HC_u_plus, color='grey')
    plt.plot(times2, total_HC_u_minus, color='grey')
